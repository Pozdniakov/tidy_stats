...
## Интерфейс для JavaScript-фреймворков: пакет `{htmlwidgets}` {#sec-html_w}

До этого мы делали только статические картинки, но в R можно делать динамические визуализации с интерактивными элементами! Делаются такие визуализации на основе *JavaScript*, в первую очередь на основе фреймворка *D3.js*. Существует пакет для R [`{htmlwidgets}`](https://www.htmlwidgets.org/), который предоставляет интерфейс для работы с *JavaScript*-визуализациями из R и вставки их в *Quarto* HTML-документы и веб-приложения *Shiny*. `{htmlwidgets}` --- это инструмент прежде всего для разработчиков R-пакетов, которые делают на его основе очень простые и удобные в использовании библиотеки для создания динамических визуализаций и прочих динамических элементов.
...
meta_2_gg <- ggplot(aes(x=Outcome_age, y=Effect_size, size=1/(SE^2), colour = Design, fill = Design), data=df %>% filter(Design != "School Age Cutoff")) +
		...
		geom_smooth(method="lm", linewidth=.5, alpha=.25) + 
...
Одно из самых распространенных средств для динамических визуализаций --- это пакет `{plotly}`.
...
if (!knitr::is_html_output(excludes = "epub")) {
  cat("\n\nК сожалению, в PDF или EPUB нельзя вставить никакой интерактивности, смотрите онлайн-версию книги)")
} else {
  ggplotly(meta_2_gg)
}
```

Не всегда это получается так, как хотелось бы, но простота этого способа подкупает: теперь наведение курсора на точки открывает небольшое окошко с дополнительной информацией о точке (конечно, если вы читаете эту книгу в PDF или EPUB, то этого не увидите).
...
Кроме `{plotly}` есть и множество других HTML-виджетов для динамической визуализации. Я рекомендую посмотреть их самостоятельно в [галерее htmlwidgets](http://gallery.htmlwidgets.org/).
